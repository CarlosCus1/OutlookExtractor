            ...
            try:
                ...
                if file_type != "Solo URLs":
                    for attachment in item.Attachments:
                        ...
                        size_ok = not is_image or attachment.Size >= MIN_IMAGE_SIZE
                        ...
                        if name_matches and extension_matches and size_ok:
                            ...
                            # Ajustar fechas: Creación = fecha del correo, Modificación = extracción
                            email_datetime = item.ReceivedTime.replace(tzinfo=None)
                            email_timestamp = pywintypes.Time(email_datetime)  # Convertir a formato de Windows
                            extraction_timestamp = time.time()  # Fecha actual

                            # Usar win32file para establecer la fecha de creación
                            handle = win32file.CreateFile(
                                file_path, win32file.GENERIC_WRITE, 0, None,
                                win32file.OPEN_EXISTING, 0, None
                            )
                            win32file.SetFileTime(handle, CreationTime=email_timestamp)
                            win32file.CloseHandle(handle)

                            # Usar os.utime para establecer la fecha de modificación
                            os.utime(file_path, (extraction_timestamp, extraction_timestamp))
...
def show_help():
    messagebox.showinfo("?", 
                        ...
                        "Los archivos tienen la fecha de creación del correo original y la fecha de modificación como la de extracción.")
